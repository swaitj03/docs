...
- `location`: Restaurant location (PostGIS geometry)
- `created_at`: Creation timestamp
- `updated_at`: Last update timestamp
- `image_url`: Restaurant image URL (nullable)

**Design Rationale**: PostGIS geometry type enables spatial queries for location-based features like "find restaurants near me" and delivery zone calculations.
...
Stores campus information for delivery zones:
...
- `geom`: Campus geometry (PostGIS geometry)
- `created_at`: Creation timestamp

**Design Rationale**: Campus-based delivery zones enable efficient order routing, driver assignment, and delivery fee calculations based on geographic boundaries.

### Spatial Reference Systems Table

Standard PostGIS table for spatial reference systems:
...
- `proj4text`: PROJ.4 string representation
...
![Database Schema](/images/schemaImageComp.svg)
...
2. Delivery address and special instructions are stored
...
#### Campus Management
When managing delivery zones:
1. Campus boundaries are defined using PostGIS geometry
2. Restaurants can be spatially queried to determine which campus they belong to
3. Delivery zones can be used for order routing and driver assignment

### Deleting Operations

**Cascade Deletion Behavior:**
- Deleting a user → Deletes driver approval, carts, cart items, and orders
...
- Deleting an order → Deletes all associated order items
...
- Implement spatial indexes (GIST) for geographic queries
