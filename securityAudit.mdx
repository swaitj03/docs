...
### Compromised packages in 1st attack and their versions:
...
### Cause of the 2nd attack:
This attack is a worm that infiltrated a single person's npm packages and then spread. It scans for and uses any valid npm tokens it finds to attempt to update and compromise an npm maintainer's packages. From there, the updated infected npm packages are installed by other developers, and the infection begins to spread once more, infecting as many packages as possible.

### Compromised packages in 2nd attack and their versions:
...
Fortunately, we have no exposed npm packages from either attack installed. As a result, no immediate action is needed.
